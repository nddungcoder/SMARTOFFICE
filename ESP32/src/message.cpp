...
//  *
//  * @param id       ID_t của thiết bị cần gửi lệnh.
//  * @param value    Giá trị float cần truyền.
//  * @param dataout  Con trỏ mảng byte đầu ra chứa gói tin đã tạo.
//  * @return         Độ dài thực tế của gói tin (số byte đã ghi).
//  */
// uint8_t Create_Message_COMMAND(ID_t id,float value, uint8_t *dataout) {
//     if (dataout == NULL) return 0;

//     uint8_t count = 0;

//     dataout[count++] = START_BYTE;
//     dataout[count++] = COMMAND;         // Group: COMMAND
//     dataout[count++] = id;            // ID thiết bị output (tùy chỉnh nếu cần)
...
void Create_Message_COMMAND(ID_t id, float value, frame_message_t messageout)
{
    ...
    uint16_t checksum = Message_Calculate_Checksum(messageout.header, HEADER_SIZE) +
                        Message_Calculate_Checksum(messageout.payload, messageout.header[2]);
